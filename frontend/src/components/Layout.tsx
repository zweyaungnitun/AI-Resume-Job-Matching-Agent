...
function Layout({ children }: LayoutProps) {
  ...
  return (
    <div className="flex h-screen bg-gradient-to-br from-blue-100 via-white to-purple-100 animate-gradient-slow">
      {/* Desktop Sidebar */}
      ...
      <div className="flex flex-1 flex-col overflow-hidden">
        {/* Header */}
        <header className="border-b border-white/70 bg-white/60 shadow-lg backdrop-blur-2xl rounded-b-3xl">
          <div className="flex items-center justify-between px-4 md:px-8 py-4">
            <div className="flex items-center gap-4">
              ...
              <h1 className="text-2xl md:text-3xl font-extrabold bg-gradient-to-r from-blue-600 via-purple-500 to-pink-500 bg-clip-text text-transparent drop-shadow-lg tracking-tight">
                JobMatch
              </h1>
            </div>
            {user && (
              <div className="hidden md:flex items-center gap-2 rounded-full border border-border/80 bg-background/60 px-3 py-1.5 shadow-sm">
                {user.picture && (
                  <img src={user.picture} alt={user.name || 'User'} className="h-7 w-7 rounded-full object-cover" />
                )}
                <span className="max-w-40 truncate text-sm text-muted-foreground font-medium">{user.name || user.email}</span>
              </div>
            )}
          </div>
        </header>
